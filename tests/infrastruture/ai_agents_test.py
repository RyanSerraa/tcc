...
def test_load_agent_success():
    ...
    with patch("src.infrastructure.ai_agents.OpenAI", return_value=mock_client) as mock_openai:
        client = Agents.load_agent("http://fake-endpoint", "fake-key")
        mock_openai.assert_called_once_with(base_url="http://fake-endpoint/api/v1", api_key="fake-key")
