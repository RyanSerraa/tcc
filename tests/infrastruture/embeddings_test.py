...
def test_embed_query_called():
    ...
    with patch("src.infrastructure.embeddings.HuggingFaceEmbeddings", return_value=mock_model):
        emb = Embeddings()  # vai usar mock_model
        result = emb.embed_query("Qual a principal causa de morte?")
        mock_model.embed_query.assert_called_once_with("Qual a principal causa de morte?")
...
def test_getContext_returns_formatted_string():
    ...
    with patch("src.infrastructure.embeddings.HuggingFaceEmbeddings", return_value=mock_model):
        ...
        contexto = emb.getContext("Qual a principal causa de morte?", "text_to_sql", mock_connection)
